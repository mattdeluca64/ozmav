...
int Viewer_SFMemCopy(unsigned int SourceBank, unsigned long SourceOffset, unsigned char * Target, unsigned long Size)
{
	int ReturnValue = 0;
	...
	case 0x00:
		...
		break;
	default:
		memcpy(Target, &ObjBuffer[SourceOffset / 4], Size);
		break;
	}

	sprintf(SystemLogMsg, "SFMemCopy: Copying 0x%04X bytes from 0x%02X|%06X\n", Size, SourceBank, SourceOffset);
